...
**The Problem:**  
Writing high-definition video to an SD card is an I/O bound task that usually "freezes" the main AI loop.  

**The Solution:**  
We use a Producer-Consumer model. The main thread handles inference and maintains a 2-second sliding window buffer in RAM. When an anomaly is detected, a background worker thread handles the FFmpeg encoding, ensuring the AI never misses a frame.  
